...
int leftprod(int grid[]);
int rightprod(int grid[]);
int upprod(int grid[]);
int downprod(int grid[]);
int ldiagprod(int grid[]);
int rdiagprod(int grid[]);
...
int main(){
    ...
    int max =0;
    int products[6];/*now we need to compare all the products, so I decided to use massive for that*/
    for(int i = 0;i <= 5;i++){
        switch ( i ){
...
int leftprod(int grid[]){
    ...
    for(int i = 3;i <= SIZE;i++){
        for(int j = 0;j <= SIZE;j++){
            if((grid[i,j]*grid[i-1,j]*grid[i-2,j]*grid[i-3]) > max){
                max = grid[i,j]*grid[i-1,j]*grid[i-2,j]*grid[i-3];
            }
        }
    }
    return max;
